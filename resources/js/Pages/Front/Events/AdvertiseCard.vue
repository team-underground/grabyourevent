	...
	<div class="md:flex items-center" :class="color">
		<div class="md:w-2/4">
			<div class="w-full relative" style="height: 420px" :class="color">
				<img :src="event.event_image_path" alt="image" class="object-fit object-cover w-full h-full" />
		...
		<div class="md:w-2/4 relative">
			...
			>
				<div>
					<heading size="display" class="tracking-wide">
						{{
						event.event_date
						}}
					</heading>
					<heading class="text-orange-600">
						{{ event.event_month }},
						{{ event.event_year }}
					</heading>
				</div>
			...
			<div class="px-8 md:px-16 py-8">
				...
				<heading size="display" class="mb-4">
					{{
					event.event_name
					}}
				</heading>
				...
				<link-to class="mt-4" :to="`/events/${event.event_slug}`" tag="inertia-link">View details</link-to>
			</div>
		</div>
	</div>
</template>
